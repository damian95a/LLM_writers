...
from LLM_writers.database import get_ollama_embeddings, OLLAMA_EMBED_API_URL
...
def search_average_similarity_per_file(
    ...
    Tuple[str, float, int]
]:  # Returns (source_file, similarity_score, num_chunks)
    print(
        f'\nSearching FILES (by averaged chunks) similar to: "{user_query[:100].strip()}..."'
    )
    print(
        f"Using Ollama model '{ollama_model_name}' for query and filtering stored chunks."
    )
    if normalize_chunks_before_averaging:
        print("Note: Chunk embeddings will be L2 normalized before averaging.")

    # 1. Embed the user query
    query_embedding_2d = get_ollama_embeddings(
        user_query, ollama_model_name, OLLAMA_EMBED_API_URL
    )
    if query_embedding_2d is None or query_embedding_2d.size == 0:
        ...
        return []
    query_vector_1d_np = query_embedding_2d[
        0
    ]  # Keep as 1D NumPy array for calculations
    ...
    print(
        f"Fetching all chunks for model '{ollama_model_name}' from '{collection_name}'..."
    )
        ...
        while True:
            ...
            if not points:
                if current_offset is None and total_fetched_points == 0:
                    print(
                        f"No points found matching filter for model '{ollama_model_name}'."
                    )
                break
            ...
            for point_record in points:
                ...
                if (
                    point_record.payload
                    and "source_file" in point_record.payload
                    and point_record.vector
                ):
                    ...
                    if isinstance(vector_data, dict):  # Named vectors
                        chunk_vec_np = np.array(
                            vector_data.get("", []), dtype=np.float32
                        ...
                        print(
                            f"Warning: Unexpected vector format for point {point_record.id}. Skipping."
                        )
                        continue

                    if chunk_vec_np.size == 0:
                        print(
                            f"Warning: Empty vector for point {point_record.id}. Skipping."
                        )
                        continue
            ...
            if current_offset is None:
                break
            print(
                f"  Fetched {total_fetched_points} points so far for model '{ollama_model_name}'..."
            )
            print(
                f"Total points fetched and processed for model '{ollama_model_name}': {total_fetched_points}"
            )
    ...
    for source_file, vectors_list in points_by_source_file.items():
        ...
        stacked_vectors = np.array(
            vectors_list
        )  # Shape: (num_chunks, vector_dimension)
        ...
        print(
            f"  Averaged {len(vectors_list)} chunks for '{source_file}'. Avg vector shape: {avg_vector.shape}"
        )
    ...
    for source_file, (
        avg_file_vector,
        num_chunks,
    ) in averaged_embeddings_per_file.items():
...
def search_similar_embeddings(
    ...
    print(
        f"Filtering results in Qdrant collection '{collection_name}' for payload.model_name = '{ollama_model_for_query}'."
    )

    # 1. Embed the user query
    query_embedding_2d = get_ollama_embeddings(
        user_query, ollama_model_for_query, OLLAMA_EMBED_API_URL
    )
    ...
    query_filter = Filter(
        must=[
            FieldCondition(
                key="model_name", match=MatchValue(value=ollama_model_for_query)
            )
        ]
    )
