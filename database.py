...
def setup_qdrant_collection(
    ...
    collection_name: str,
    vector_dim: int,
    current_ollama_model_name: str,
    recreate_if_needed: bool = False,
):
    print(
        f"Setting up Qdrant collection '{collection_name}' with vector dimension {vector_dim}..."
    )
    try:
        ...
        if existing_vector_size and existing_vector_size != vector_dim:
            ...
            if recreate_if_needed:
                ...
                print(
                    "This will lead to errors during upsert. Please use a different collection name, "
                    "ensure your model matches the collection, or enable recreation."
                )
                exit(1)
        else:
            print(
                f"Collection vector size {existing_vector_size or 'N/A (no default vector)'} is compatible or collection is new."
            )

    except (
        Exception
    ) as e:  # Catches Qdrant errors if collection doesn't exist or other API issues
        if "not found" in str(e).lower() or (
            "status_code=404" in str(e).lower() if hasattr(e, "status_code") else False
        ):
            ...
            print(
                f"Error during Qdrant collection check/setup for '{collection_name}': {e}"
            )
...
def get_ollama_embeddings(
    text: str, model_name: str, api_url: str
):  # -> np.ndarray | None:
    try:
        ...
        if "embedding" not in response_json:
            raise ValueError(
                f"Ollama API response for model '{model_name}' no 'embedding'. Resp: {response_json}"
            )
        embedding_1d = np.array(response_json["embedding"], dtype=np.float32)
        return embedding_1d.reshape(1, -1)
    except requests.exceptions.RequestException as e:
        print(
            f"Error Ollama API ({api_url}) model '{model_name}': {e.response.text if hasattr(e,'response') and e.response else e}"
        )
        return None
...
def check_ollama_status_and_get_dim(model_to_check: str):
    ...
    try:
        ...
        print(
            f"Error: Ollama server not reachable at {OLLAMA_API_BASE_URL}. Ensure Ollama is running."
        )
        print(f"Details: {e}")
        exit(1)
    ...
    try:
        response = requests.post(
            f"{OLLAMA_API_BASE_URL}/api/show", json={"name": model_to_check}, timeout=10
        )
        if response.status_code == 404:
            print(
                f"Error: Ollama model '{model_to_check}' not found. Pull or create it (e.g., 'ollama pull {model_to_check}')."
            )
            exit(1)
        response.raise_for_status()
        ...
        sample_embedding = get_ollama_embeddings(
            "sample text", model_to_check, OLLAMA_EMBED_API_URL
        )
        if (
            sample_embedding is not None
            and sample_embedding.ndim == 2
            ...
            print(
                f"Error: Could not determine vector dimension for '{model_to_check}'. Exiting."
            )
